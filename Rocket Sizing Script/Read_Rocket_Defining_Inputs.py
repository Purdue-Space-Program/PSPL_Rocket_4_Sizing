...
# Input: N/A (but needs to be in the same folder as the Rocket-Defining Inputs Excel file)
# Output:
...
def read_inputs():
    ...
    with pd.ExcelFile('Rocket-Defining Inputs.xlsx') as RDIs:
        continuous_inputs = pd.read_excel(RDIs, 'Continuous Inputs', index_col=0) # Dataframe containing continuous inputs. Columns are different inputs, rows are start-stop-step values
        prop_combos = pd.read_excel(RDIs, 'Propellant Combinations', index_col=0) # Dataframe containing propellant combination options. Rows are different combinations, columns are oxidizers and fuels
        tank_walls = pd.read_excel(RDIs, 'Tank Walls', index_col=0) # Dataframe containing tank wall options. Rows are different walls, columns are wall parameters
        copvs = pd.read_excel(RDIs, 'COPVs', index_col=0) # Dataframe containing COPV options. Rows are different COPVs, columns are COPV parameters
    ...
    possible_rockets_df = pd.DataFrame(possible_rockets, index=RIDs, columns=['Propellant combination', 'O:F (mass)'] + list(non_prop_inputs.keys())) # Dataframe containing all possible rockets. Rows are rockets, columns are inputs
